...
export function renderTabs(onRender) {
  ...
  state.tabs.forEach((tab) => {
    ...
    li.className = `tab-item ${tab.id === state.selectedTabId ? 'active' : ''}`;

    li.innerHTML = `
      <button class="tab-select" type="button">${escapeHtml(tab.name)}</button>
      <div class="tab-actions">
        <button data-action="rename" type="button">이름변경</button>
        <button data-action="delete" type="button">삭제</button>
      </div>
    `;

    li.querySelector('.tab-select').addEventListener('click', () => {
...
export function renderEditor() {
  ...
  contentEl.contentEditable = String(!disabled);
