import { validateDeadline } from './deadline.js';
 ...
 */
export async function extractTodosWithAI(noteHtml, noteCreatedAt = '') {
  ...
  try {
    response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${encodeURIComponent(apiKey)}`,
      {
        ...
        body: JSON.stringify({
          contents: [{ parts: [{ text: buildPrompt(content, todayStr, noteDate) }] }],
  ...
  return parseAIResponse(text, today);
}
...
function buildPrompt(content, today, noteDate) {
  return `다음 노트에서 사용자가 반드시 수행해야 할 액션 아이템만 추출하세요.
...
function parseAIResponse(text, fromDate) {
